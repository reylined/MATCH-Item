...
def train_match(fixed_param, train_data_tensors):
...
for i_sim in range(n_sim):
    ...
    train_long, train_mask, train_e, train_t, obs_time = get_tensors(df=train_data.copy(),
                                                                     ...
                                                                     obstime="obstime",
                                                                     roundnum=1)
    train_long = ordinalOHE(train_long.long(), ordinal=False, n_cat=4).permute(0,1,3,2)
    ...
    test_long, test_mask, test_e, test_t, obs_time = get_tensors(df=test_data.copy(),
                                                                     ...
                                                                     obstime="obstime",
                                                                     roundnum=1)
    test_long = ordinalOHE(test_long.long(), ordinal=False, n_cat=4).permute(0,1,3,2)
    ...
    model = train_match(fixed_param, train_data_tensors)
