...
dat = dat %>% select(id, obstime, event, time, sumscore)
dat = dat %>% filter(obstime < time)
...
# mvlmefit = mvglmer(list(sumscore ~ obstime + (obstime | id)), data = train_data,
#                    families = list(gaussian))
lmefit = lme(sumscore ~ obstime, random = ~ obstime | id, data = train_data)


# Survival Submodel
survfit = coxph(Surv(time,event) ~ 1, data=train_data%>%filter(obstime==0), model=TRUE, x=TRUE)

# Joint Model
#jmfit <- mvJointModelBayes(mvlmefit, survfit, timeVar = "obstime")
jmfit <- jointModelBayes(lmefit, survfit, timeVar = "obstime")
...
filename = paste0("Sim_predictions/JM_sumscore/JM_sumscore_",i_sim,".rdata")
